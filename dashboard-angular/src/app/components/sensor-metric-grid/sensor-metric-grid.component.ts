...
import { SensorMetricCardComponent } from '../sensor-metric-card/sensor-metric-card.component';
import { TelemetryData } from '../../services/telemetry.service';

@Component({
  selector: 'app-sensor-metric-grid',
  standalone: true,
  imports: [CommonModule, SensorMetricCardComponent],
  templateUrl: './sensor-metric-grid.component.html',
  styleUrls: ['./sensor-metric-grid.component.css']
})
export class SensorMetricGridComponent {
  ...
  getValue(id: string): number | null | undefined {
    if (!this.latestData) return null;
    return (this.latestData as any)[id];
  }
}
